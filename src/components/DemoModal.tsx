...
import { X, CheckCircle2, ShieldCheck, ArrowRight, Loader2, Sparkles, Volume2 } from 'lucide-react';
...
export const DemoModal: React.FC<DemoModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.companyName) {
      localStorage.setItem('smit_demo_company', formData.companyName);
    }
    ...
    >
      ...
      <div className="relative w-full max-w-lg my-8 rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xl p-6 sm:p-8 z-10 animate-in zoom-in-95 duration-200">
        ...
        {!isSuccess ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    placeholder="ООО «Телеком-Сети»"
                    value={formData.companyName}
                    onChange={(e) => {
                      const val = e.target.value;
                      setFormData({ ...formData, companyName: val });
                      if (val) localStorage.setItem('smit_demo_company', val);
                    }}
                    className="w-full px-3.5 py-2.5 rounded-xl border text-sm text-slate-900 dark:text-white bg-slate-50 dark:bg-slate-800/60 border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
                  />
                </div>
              </div>
          ...
          /* Success Screen with Subtle Animations and Sound Feedback */
          <div className="text-center py-6 space-y-4 animate-in zoom-in-95 duration-200">
            {/* Animated Icon with Glow Ring and Particles */}
            ...
            <div>
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 dark:bg-emerald-950/50 border border-emerald-200 dark:border-emerald-800 text-[11px] font-semibold text-emerald-700 dark:text-emerald-400 mb-2">
                <Sparkles className="w-3.5 h-3.5" />
                <span>Демо-стенд готов • Стенд зарезервирован</span>
              </div>
            ...
            <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 text-xs text-left space-y-1.5 max-w-xs mx-auto text-slate-600 dark:text-slate-400 font-mono">
              <div className="text-[11px] font-sans font-bold text-slate-700 dark:text-slate-300 mb-1">
                Параметры окружения:
              </div>
            ...
            <div className="flex items-center justify-center gap-3 pt-2">
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium text-slate-500 dark:text-slate-400 hover:text-emerald-500 dark:hover:text-emerald-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer"
